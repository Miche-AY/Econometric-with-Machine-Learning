...
institute: "Quantitative Economics Masters and Ph.D. in Economics Program \n \n
...
format: 
  beamer:
    ...
    colortheme: "seagull"
    toc: true
    toc-title: "Trame de présentation"
header-includes:
  - \AtBeginSection{}
  - \AtBeginPart{}
---
...
\begin{theorem}[29.2 BH Econ]
Dans le modèle de régression linéaire, si $0<\lambda<2\underline{\sigma}^2/\beta'\beta$,
$$
...
- Introduits par *[Breiman, Friedman, Olshen et Stone (1984)](https://rafalab.dfci.harvard.edu/pages/649/section-11.pdf)*, ils sont également connus sous l’acronyme **CART** (*Classification and Regression Trees*). 
...
- Ojectif : estimer la fonction de régression conditionnelle : $$m(x) = \mathbb{E}[Y | X = x]$$
...
1. Sélectionner une taille minimale de nœud $N_{\text{min}}$. C'est le nombre minimal d'observations dans chaque feuille.  
...
- L'estimateur de sélection $\hat \theta_{\text{pms}} = h(\hat \theta)$ est la transformation en seuil dur $h(t)$ appliquée à $\hat \theta$, l'estimateur par bagging $\hat \theta_{\text{bag}} = g(\hat \theta)$ est la transformation lissée $g(t)$ appliquée à $\hat \theta$.

- L'application la plus courante du bagging concerne les arbres de régression. Les arbres ont une structure similaire à notre estimateur de sélection $\hat \theta_{pms}$ et devraient donc entraîner une réduction de l'EQM par rapport à l'estimation par arbre de régression.
...
- L'estimateur « leave i out » du bagging, $\hat m_{-i}(x)$, de $m(x)$ est obtenu en moyennant uniquement sur ce second ensemble (les $37 \%$ qui excluent l'observation).
- L'erreur hors échantillon est alors $\tilde e_i = Y_i - \hat m_{-i}(X_i)$. Le CV croisée hors échantillon est $\sum_{i=1}^{n} \tilde e_i^2$. Celui-ci peut être utilisé comme estimateur de l'EQMP hors échantillon. 
...
Wager, Hastie et Efron (2014) proposent des estimateurs de $V_n(x) = \text{var}(\hat m_{\text{bag}}(x))$. Soit $N_{ib}$ le nombre de fois où l'observation $i$ apparaît dans l'échantillon bootstrap $b$, et $N_i = \frac{1}{B} \sum_{b=1}^{B} N_{ib}$.

### L'estimateur **jackknife infinitésimal** (EJI) de $V_n$

$$
\begin{aligned}
\hat V_n(x) &= \sum_{i=1}^{n}\text{cov}^* \left(N_i, \hat m_{\text{bag}}(x)\right)^2 \\ 
\hat V_n(x)&=\sum_{i=1}^{n} \left[ \frac{1}{B} \sum_{b=1}^{B} (N_{ib} - N_i) (m_b(x) - m_{\text{bag}}(x)) \right]^2.
\end{aligned} \tag{EJI}
$$
...
- La modification apportée par les forêts aléatoires consiste à **décorréler** les arbres de régression bootstrap en introduisant une **randomisation supplémentaire**.
...
- Cette méthode sélectionne les poids d’agrégation des modèles en minimisant un critère de validation croisée, sous la contrainte que $\sum_i w_i = 1$ avec $w_i \ge 0$. 
...
Cette transformation élimine le régresseur $X$ ainsi que le coefficient de grande dimension $\beta$. Les modèles $(2)$ et $(3)$ spécifient $\mathbb{E}[Y\mid X]$ et $\mathbb{E}[D\mid X]$ comme des fonctions linéaires de $X$.
...
3. **Estimation finale** : Soit $\hat \theta_{PR}$ le coefficient des MCO de la régression de $\hat U$ sur $\hat V$.  
...
- Sa sensibilité par rapport à $\beta$ est donnée par sa dérivée évaluée aux vrais coefficients, qui est non nul lorsque $D$ et $X$ sont corrélés. 
...
- Sa sensibilité par rapport à $\beta$ est égale à zéro et est donc non corrélé avec $X$.
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.3\textwidth]{"C:/Users/HP/Pictures/Camera Roll/photofunky.png"}
\end{figure}
